...
- Manifests: **`public/manifest.webmanifest`** (storefront, **`id`:** **`pwa-storefront`**) and **`public/admin/manifest.webmanifest`** (staff, **`id`:** **`pwa-staff`**). Distinct **`id`** values stop Chromium/Edge from treating the two PWAs as one app that “updated its name.” Paths use **`./`** / **`../`** for **`start_url`**, **`scope`**, and icons under **`NEXT_PUBLIC_BASE_PATH`**. Staff **`start_url: "./"`** resolves to **`…/admin/`** (trailing slash). For GitHub Pages, that URL must map to **`admin/index.html`** in the exported site — **`next.config.ts`** sets **`trailingSlash: true`** whenever **`output: "export"`** runs so Next emits **`out/admin/index.html`** instead of **`out/admin.html`** (which caused a real **404** and your **`not-found.tsx`** on **`/VISO/admin/`**). There is no root **`app/manifest.ts`** — it broke the staff manifest link in prerendered admin HTML during static export.
...
- **GitHub Pages (this repo):** the workflow `.github/workflows/deploy-pages.yml` builds a static export with `NEXT_PUBLIC_BASE_PATH=/VISO` and `NEXT_PUBLIC_SITE_URL=https://cowebslb.github.io/VISO`, uploads the **entire** `out/` tree as the Pages artifact, and deploys via GitHub Actions. In the repo **Settings → Pages**, set **Source** to **GitHub Actions** (not “Deploy from a branch,” which only shows the README). Add **repository secrets**: `WHATSAPP_ORDER_NUMBER` (digits only), `NEXT_PUBLIC_SUPABASE_URL`, and `NEXT_PUBLIC_SUPABASE_ANON_KEY` (for catalog static paths and client-side checkout/admin). In **Supabase → Authentication → URL configuration**, set **Site URL** to **`https://cowebslb.github.io/VISO`** so auth redirects and emails stay under the app (see **`docs/admin-auth-and-login.md`**). If **`/VISO/admin/`** 404s after a deploy, confirm the live export includes **`admin/index.html`** (static export + **`trailingSlash`** — see **PWA** above); the workflow is not omitting **`admin/`**.
